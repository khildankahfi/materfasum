...
@section('title', 'Peta Wilayah GIS')
@section('page-title', 'Peta Wilayah GIS')
@section('breadcrumb')
...
@section('content')
<div class="mb-4">
    <h4 class="font-extrabold text-xl text-slate-800 mb-1">Peta Sebaran Laporan Warga 📍</h4>
    <p class="text-slate-500 font-semibold text-xs mb-0">Pantau persebaran laporan kerusakan fasilitas umum di seluruh wilayah kota secara visual.</p>
</div>
    ...
    <div class="col-12 col-lg-3">
        <div class="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm h-full flex flex-col justify-content-between">
            <div>
                ...
                <div class="space-y-2 mb-4 d-grid gap-2">
                    <button class="btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 d-flex items-center justify-between transition-all w-100" id="filter-all" onclick="filterMap('all')">
                        <span>Semua Laporan</span>
                        <span class="badge bg-blue-600 text-white rounded-pill" id="count-all">0</span>
                    </button>
                    <button class="btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 border border-slate-100 d-flex items-center justify-between transition-all w-100" id="filter-jalan" onclick="filterMap('Jalan')">
                    ...
                    <button class="btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 border border-slate-100 d-flex items-center justify-between transition-all w-100" id="filter-lampu" onclick="filterMap('Lampu Jalan')">
                        <span class="d-flex align-items-center gap-2">
                    ...
                    <button class="btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 border border-slate-100 d-flex items-center justify-between transition-all w-100" id="filter-taman" onclick="filterMap('Taman')">
                        <span class="d-flex align-items-center gap-2">
                    ...
                    <button class="btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 border border-slate-100 d-flex items-center justify-between transition-all w-100" id="filter-drainase" onclick="filterMap('Drainase')">
                        <span class="d-flex align-items-center gap-2">
                ...
                <div class="space-y-2.5 d-flex flex-column gap-2">
                    <div class="d-flex align-items-center gap-3 text-xs font-bold text-slate-500 px-1">
                        <span class="bg-amber-100 text-amber-600 rounded-xl h-7 w-7 d-flex align-items-center justify-center border border-amber-200/50">
                            <i class="bi bi-hourglass-split"></i>
                        </span>
                        <span>Menunggu Validasi</span>
                    </div>
                    <div class="d-flex align-items-center gap-3 text-xs font-bold text-slate-500 px-1">
                        <span class="bg-blue-100 text-blue-600 rounded-xl h-7 w-7 d-flex align-items-center justify-center border border-blue-200/50">
                            <i class="bi bi-gear-wide-connected"></i>
                        </span>
                        <span>Sedang Diproses</span>
                    </div>
                    <div class="d-flex align-items-center gap-3 text-xs font-bold text-slate-500 px-1">
                        <span class="bg-emerald-100 text-emerald-600 rounded-xl h-7 w-7 d-flex align-items-center justify-center border border-emerald-200/50">
...
<script>
    ...
    const map = L.map('map-container').setView([-7.1539, 112.6561], 12);
    ...
    function getMarkerColor(status) {
        ...
        if (status === 'diproses') return '#0284c7';
            ...
            if (report.latitude && report.longitude) {
                ...
                const popupContent = `
                    <div class="p-3 bg-white max-w-[280px]">
                        ...
                        <a href="${report.url}" class="btn btn-primary btn-sm w-100 rounded-lg text-xs font-bold py-1.5 text-decoration-none text-white text-center d-block">
    ...
    function filterMap(category) {
        ...
        categoriesIds.forEach(id => {
            ...
            if (id === category.toLowerCase() || (category === 'Lampu Jalan' && id === 'lampu') || (category === 'all' && id === 'all')) {
                btn.className = "btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 d-flex items-center justify-between transition-all w-100";
            } else {
                btn.className = "btn btn-sm text-start px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 border border-slate-100 d-flex items-center justify-between transition-all w-100";
            }
        });
        renderMapMarkers();
    }
